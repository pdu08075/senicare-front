...
export default function CSWrite() {
    ...
    // state: 이미지 입력 장소 //
    const imageInputRef = useRef<HTMLInputElement | null> (null);               // '(null)': 초기값 null로 지정

    // state: 프로필 미기보기 url 상태 //
    ...
    const onImageInputChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (!files || !files.length) return;                                    // '!files.length'는 'files.length가 0이라면' 이라는 뜻
        ...
        fileReader.onloadend = () => {                                          // 'fileReader가 끝이 난다면' 이라는 뜻
        ...
        <div id='cs-write-wrapper'>
            <div className='main'>
                <div className='profile-image' style={{ backgroundImage: `url(${previewUrl})` }} onClick={onProfileImageClickHandler}>
                    <input ref={imageInputRef} style={{ display: 'none' }} accept='image/*' type='file' onChange={onImageInputChangeHandler} />
                </div>
                <div className='input-container'>
                    ...
                    <div className='input-box'>
                        ...
                        <input className='input' value={birth} placeholder='생년월일을 입력해주세요.' onChange={onBirthChangeHandler} />
                    </div>
                    <div className='input-box'>
                        <div className='input-label'>담당자</div>
                        <input className='input' placeholder='담당자 이름을 입력하세요.' />
                    ...
                    <div className='input-box'>
                        ...
                        <input className='input' placeholder='주소를 선택하세요.' />
                        <div className='button-box'>
                            <div className='button disable'>검색</div>
                        </div>
                    </div>

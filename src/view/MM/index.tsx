...
function TableRow({ tool, onUpdateButtonClickHandler, getToolList }: TableRowProps) {
    ...
    const onDeleteButtonClickHandler =() => {
        const isConfirm = window.confirm('정말로 삭제하시겠습니까?');                                   // '확인'과 '취소'가 있는 alert 창 띄우기
...
export default function MM() {
    ...
    // function: tool list 불러오기 함수 //                                                                                         // 수정 바로 반영
    const getToolList = () => {
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
...
function deleteToolResponse(value: ResponseDto | null): ResponseDto | PromiseLike<ResponseDto | null> | null {
